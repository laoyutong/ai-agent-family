...
本文档系统整理对 **chatbot-memory** 的架构诊断：**可优化的现有链路**、**有复杂度的功能新增方向**，以及**优先级建议**。分析基于仓库当前实现（Vite + Express、DeepSeek 流式、会话 JSON 持久化、MCP 代码沙盒、熵过滤与记忆折叠等）。
...
一条用户消息在最坏情况下可能触发 **多次非流式调用** 后才开始流式输出：

```
用户消息
  → [1] 熵过滤 (filterDialogueByEntropyPrinciple) — 非流式
  → [2] MCP 路由判断 (shouldUseMcpSandboxForTurn) — 非流式
...
- **熵过滤与路由判断并行化**：两者输入相对独立，可考虑 `Promise.all` 并发（需注意与 `messages` 组装顺序的一致性）。
- **路由判断本地化**：在调用 LLM 前先做关键词/正则预判（如「读文件」「查找」「搜索」等直接走 MCP），仅在不确定时再调用路由模型。
- **MCP 首轮体验**：代码生成阶段可向前端推送进度类事件（见下文「流式 MCP 进度反馈」），降低空白等待感。

---

### 2.2 会话持久化的单文件瓶颈

当前所有会话序列化在 **单个 JSON 文件**（默认 `~/.chatbot-memory/sessions.json`）中，变更时 **全量** `JSON.stringify` + 临时文件写入 + `rename`。

**问题：**

- 随会话数量与单会话轮次增长，序列化与写入成本线性上升。
- 写放大：任意会话一次变更即重写整库。

**优化方向：**

- **按会话分文件**：`sessions/<uuid>.json`，仅写变更会话。
- **增量 / WAL**：追加变更日志，定期合并（实现复杂度较高）。
- **SQLite**：结构化存储、事务、按需读写（需引入依赖与迁移策略）。
...
| 优先级 | 项目 | 类型 | 理由 |
|--------|------|------|------|
| P0 | LLM 调用扇出并行化 / 路由本地化 | 优化 | 直接降低首字延迟，改动相对可控 |
...
| P2 | 会话持久化分文件或 SQLite | 优化 | 会话少时单文件可接受，增长后收益大 |
...
| `src/server/session-store.ts` | JSON 全量持久化 |
